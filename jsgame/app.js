...
add.addEventListener("click", (e) => {
  // e.preventDefault() 去掉表單原本送出遠端的功能
  // e.target.parentElement 取得上一層
  e.preventDefault();
  ...
  trashButton.addEventListener("click", (e) => {
    ...
    todoItem.style.animation = "scaleDown 0.6s forwards";
  });
...
function mergeTime(arr1, arr2) {
  ...
  while (i < arr1.length && j < arr2.length) {
    ...
    if (Number(arr1[i].todoYear) > Number(arr2[j].todoYear)) {
      ...
      // 2."年"數值相同、比較"月"
      if (Number(arr[i].todoMonth) > Number(arr2[j].todoMonth)) {
        result.push(arr2[j]);
        j++;
      } else if (Number(arr[i].todoMonth) < Number(arr2[j].todoMonth)) {
        result.push(arr1[i]);
        i++;
      } else if (Number(arr[i].todoMonth) == Number(arr2[j].todoMonth)) {
        // 3."月"數值相同、比較"日"
...
//查看排序 console.log(mergeSort(JSON.parse(localStorage.getItem("list"))));
